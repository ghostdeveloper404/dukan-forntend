...
const Flashsales = () => {
  ...
  useEffect(() => {
    const targetDate = new Date('2025-12-31T23:59:59'); // Set your target date and time here
  ...
  return (
    <div  className='bg-[#f0eeee]  w-full h-full border-b-2 '> 

      <div className="flex content-left pl-20  items-center mt-10">
        <div className="bg-red-500 w-6 h-10 rounded-md border-r-2 border-red-600 mr-2"></div>
        <h2 className="text-2xl  text-red-500">Today&apos;s</h2>
      </div>
      <div className="flex content-left justify-between pl-16 items-center mt-5">
        <div className="flex content-left m-2  items-center ">
          <h1 className="text-4xl text-black font-bold ">Flash Sales</h1>
        </div>
        <div className="flex justify-center items-center ">
          <div className="text-center">
            <div className='font-bold m-1 text-sm ' >Days</div>
            <div className="text-3xl font-bold">{timeLeft.days}</div>
          </div>
          <div className="text-4xl text-red-600 mt-7 rounded-2xl font-bold">:</div>
          <div className="text-center">
            <div className='font-bold m-1 text-sm' >Hours</div>
            <div className="text-3xl font-bold">{timeLeft.hours}</div>
          </div>
          <div className="text-4xl text-red-600 mt-7 rounded-2xl font-bold">:</div>
          <div className="text-center">
            <div className='font-bold m-1 text-sm' >Minutes</div>
            <div className="text-3xl font-bold">{timeLeft.minutes}</div>
          </div>
          <div className="text-4xl text-red-600 mt-7 rounded-2xl  font-bold">:</div>
          <div className="text-center">
            <div className='font-bold m-1 text-sm'>Seconds</div>
        ...
        <div className='flex relative justify-center items-center mr-16 '>
          <button
              onClick={() => {
                ...
                console.log(scrollableGrid);
                if (scrollableGrid) {
                  scrollableGrid.scrollBy({ left: -200, behavior: 'smooth' });
                }
              }}
      ...
      <div className='flex  justify-center relative ml-16 mr-6 items-center mt-4   '>
        <div id="scrollable-grid"
         ...
         className='flex scroll-smooth overflow-x-scroll scrollbar-hide   scrollbar-hide flex-shrink-0 scrollbar-none  gap-4   '>
          {Products.map((product) => (
            <div key={product.id}

            style={{ minWidth: '250px', maxWidth: '280px', height: '460px' }} // Fixed card dimensions
            ...
            className=" group  flex-shrink-0 flex-col items-center justify-center p-5 bg-white shadow-lg rounded-lg  hover:shadow-xl transition-shadow duration-300 mb-5  relative scrollbar-hide scrollbar-none  ">
             ...
             <img
                ...
                width={400}
                height={200}
                className=" p-2   w-auto h-80  object-cover rounded-lg bg-gray-300 rounded-t-lg "
              />
              <Link
                href={product.link}
              ...
              <div className=' m-2 flex gap-2 flex-row items-center justify-left '>
                <p className="text-xl text-red-500 "> ${product.offprice}</p>
                <p className="text-gray-400 text-xl line-through "> ${product.price}</p>
              </div>
         ...
         <div className='m-5 left-1/3 relative bg-red-500 w-40 p-2 h-10 rounded-md border-r-2 border-red-600 flex justify-center items-center '>
          <Link href="/allprodcts" className='text-lg  text-white'>View All Product</Link>
         </div>
    </div>
